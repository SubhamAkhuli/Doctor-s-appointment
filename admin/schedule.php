                    ...
                    if($_POST){
                        ...
                        if(!empty($_POST["sheduledate"]) && !empty($_POST["docid"]) ){
                            ...
                            if(!empty($_POST["docid"])){
                                $docid=$_POST["docid"];
                                $sqlmain= "select schedule.scheduleid,schedule.title,doctor.docname,schedule.scheduledate,schedule.scheduletime,schedule.nop from schedule inner join doctor on schedule.docid=doctor.docid where doctor.docid=$docid ";
                            }
                            elseif (!empty($_POST["sheduledate"])){
                            $sheduledate=$_POST["sheduledate"];
                            $sqlmain= "select schedule.scheduleid,schedule.title,doctor.docname,schedule.scheduledate,schedule.scheduletime,schedule.nop from schedule inner join doctor on schedule.docid=doctor.docid where schedule.scheduledate='$sheduledate'";
                            }
                            else{
                                $sqlmain= "select schedule.scheduleid,schedule.title,doctor.docname,schedule.scheduledate,schedule.scheduletime,schedule.nop from schedule inner join doctor on schedule.docid=doctor.docid  order by schedule.scheduledate asc";
            ...
            <table border="0" width="100%" style=" border-spacing: 0;margin:0;padding:0;margin-top:25px; ">
                ...
                <tr>
                    <td colspan="4" style="padding-top:10px;width: 100%;" >
                        ...
                        <p class="heading-main12" style="margin-left: 45px;font-size:18px;color:rgb(49, 49, 49)">All Sessions (<?php echo $result->num_rows; ?>)</p>
                    </td>
